...
export const WorkspaceChat: React.FC<WorkspaceChatProps> = ({ workspaceId, currentUser, members, onClose }) => {
  ...
  useEffect(() => {
    ...
    const msgChannel = supabase.channel(`chat-room-${workspaceId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'workspace_messages', filter: `workspace_id=eq.${workspaceId}` }, (payload) => {
          if (payload.eventType === 'DELETE') {
              setMessages(prev => prev.filter(m => m.id !== payload.old.id));
          } else {
              fetchMessages(); // Refresh for inserts/updates to get relations
          }
      })
  ...
  const insertMention = (username: string) => {
      ...
      const afterCursor = inputText.slice(inputRef.current?.selectionStart || 0);
  ...
  return (
    <div className="absolute bottom-20 right-0 w-[90vw] md:w-[800px] h-[600px] max-h-[80vh] bg-white border-4 border-slate-800 rounded-3xl shadow-[16px_16px_0px_0px_#1E293B] flex overflow-hidden animate-in zoom-in-95 origin-bottom-right z-50">
        ...
        <div className="flex-1 flex flex-col min-w-0 border-r-2 border-slate-200 relative">
            ...
            {/* MESSAGES */}
            <div className="flex-1 overflow-y-auto p-4 space-y-6 dot-grid bg-slate-50/30" ref={scrollRef}>
                {messages.length === 0 && (
                    ...
                    <div className="absolute bottom-full left-4 mb-2 bg-white border-2 border-slate-800 rounded-xl shadow-pop z-50 w-64 overflow-hidden max-h-48 overflow-y-auto animate-in fade-in slide-in-from-bottom-2">
                        {filteredMembers.length > 0 ? (
                            filteredMembers.map(m => (
                                <button
                                    key={m.id}
                                    onClick={() => insertMention(m.users?.username || 'user')}
